...
(function() {
    ...
    // To add more alternatives in the future, simply append new filenames to the arrays below.
    const SAMPLE_FILES = {
        'arp': ['arp.wav'],
        'bass': ['bass.wav'],
    ...
    Object.keys(SAMPLE_FILES).forEach(key => {
        ...
        if (!window.wavSettings[key]) {
            window.wavSettings[key] = { volume: 0.8, pan: 0.0 };
        }
    });
    ...
    class SamplerVoice {
        ...
        start() {
            ...
            const userVol = window.wavSettings[this.sampleKey] ? window.wavSettings[this.sampleKey].volume : 0.8;
